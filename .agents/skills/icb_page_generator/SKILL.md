...
11. **AI 雙模式任務與 templates/ 定位：** `templates/` 是「靈感庫 + 積木庫」，不是「直接成品庫」。嚴禁直接複製 templates/ 任一檔案完整結構作為最終輸出。1.【套版模式 `/page`】允許從 `templates/` 和 `page_templates.md` 配方直接組裝，替換文案圖片即可。2.【創作模式 `/create`】`templates/` 僅作結構寫法參考，AI 必須重新設計 section 順序、比例、視覺重心，且先出文字骨架確認後才生成 XML+SCSS。3.【外部佈景翻譯 `/clone`】拆解外部版型，優先找 `templates/` 可套用積木，若無則生成全新結構。
