...
19. **/clone 1:1 優先：** 除非使用者明確要求等價替換，/clone 預設以 1:1 結構還原為優先（含區塊順序與核心佈局），再視 Odoo 限制做必要調整。
20. **/clone 結構優先：** 先解析原站 DOM+CSS 的呈現邏輯，能用 Odoo/Bootstrap 既有結構（row/col/容器）還原左右/層次者，優先用結構取代 CSS，避免多寫 SCSS；只有結構無法達成時才補 SCSS。
