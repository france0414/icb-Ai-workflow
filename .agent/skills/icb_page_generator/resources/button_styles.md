...
> [!WARNING]
> **`btn-style.xml` 中的 `s_custom_btn01`、`s_custom_btn02` 等 class 僅用於模板預覽頁面的樣式隔離！**
...
完整的結構、SCSS 與預覽展示在 `templates/btn-style.xml` 中。

> [!NOTE]
> 下方 SCSS 範例為**精簡版核心邏輯**，實際部署時請從 `btn-style.xml` 的 `<pre id="scss-code">` 中複製完整版（含 o_cc1~o_cc5 色彩變數、尺寸適配、編輯器模式處理）。
